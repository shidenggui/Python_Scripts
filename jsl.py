...
# jsl = JSL()
# 初始化时传入利率范围和最小交易量
# jsl = JSL(range=[3, 3.2], minvolume=5000)
# 设置分级A的利率范围
# jsl.range = [4]
# 设置之后分级A的最小交易量
# jsl.minvolume = 1000
...
class JSL(object):
    ...
    def __init__(self, range = [], minvolume = 0):
        self.range = range
    ...
    def getFunda(self):
        ...
        # 检查是否设置过滤条件
        if len(self.__range) or self.__minvolume:
            # 过滤掉不符合条件的A
            for k in list(alladata):
                cell = alladata[k]
                if not cell['coupon_descr_s'][1:4] in ''.join(self.__range) or float(cell['funda_volume']) < self.__minvolume:
                    alladata.pop(k)
